...
con.query(`SELECT * FROM inventory WHERE id = ${message.author.id}`, (err, rows) => {
	  ...
	  if(rows.length == 0) return message.channel.send("Tu n'es pas entré dans le jeu, fais +i pour t'inscrire.")
	 	...
	 	let level = rows[0].niveau;
	        ...
	        if(rows[0].niveau < nxtLvl) con.query(`UPDATE inventory SET niveau = ${nxtLv}, xp = 0, maxmana = ${parseInt(rows[0].maxmana)+(5*nxtLvl)}, attack = ${parseInt(rows[0].attack)+(3*nxtLvl)}, defense = ${parseInt(rows[0].defense)+(3*nxtLvl)} , pui = ${parseInt(rows[0].attack + rows[0].defense) + (6*nxtLvl)} WHERE id = ${message.author.id}`)
	 	...
	 	let level = rows[0].niveau;
	        ...
	        if(rows[0].niveau < nxtLvl) con.query(`UPDATE inventory SET niveau = ${nxtLvl}, xp = 0, maxmana = ${parseInt(rows[0].maxmana)+(5*nxtLvl)}, attack = ${parseInt(rows[0].attack)+(3*nxtLvl)}, defense = ${parseInt(rows[0].defense)+(3*nxtLvl)}, pui = ${parseInt(rows[0].attack + rows[0].defense) + 6} WHERE id = ${message.author.id}`)
