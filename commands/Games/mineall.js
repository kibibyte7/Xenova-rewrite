...
run(message, args, level, con) {
   ...
   if(chance > 70) wood();
	 ...
	 if(r[0].niveau < nxtLvl) con.query(`UPDATE inventory SET niveau = ${nxtLvl}, xp = 0, maxmana = ${parseInt(r[0].maxmana)+(5*nxtLvl)}, attack = ${parseInt(r[0].attack)+(3*nxtLvl)}, defense = ${parseInt(r[0].defense)+(3*nxtLvl)}, pui = ${parseInt(rows[0].attack + rows[0].defense) +(6*nxtLvl)} WHERE id = ${message.author.id}`)
