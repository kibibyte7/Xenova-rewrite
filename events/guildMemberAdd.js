...
module.exports = class {
  ...
  async run (member) {
    ...
    ctx.fillText('Bienvenue sur le serveur,', canvas.width / 3.1, canvas.height / 3.7);

    ctx.font = this.client.applyText(canvas, `${member.user.displayName}`) && '40px NFS';
    ctx.fillStyle = '#ffffff';
    ctx.fillText(member.displayName, canvas.width / 3.1, canvas.height / 1.8);
    
    ctx.font = `Amuses toi bien, nous sommes ${member.guild.members.size} Membres !` && '16px NFS';
    ctx.fillStyle = '#ffffff';
    ctx.fillText(`Amuses toi bien, nous sommes ${member.guild.members.size} Membres !`, canvas.width / 3.1, canvas.height / 1.2);
